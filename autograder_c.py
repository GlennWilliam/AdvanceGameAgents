...
import game_types
import lezhiliu_KInARow as agent_module
...
def test_zobrist_hits_on_fiar(agent):
    """
    Try to force some repeated states in a 5-in-a-row (7x7) game
    so we can see if the Zobrist transposition table ever hits.
    """

    # Construct a partial 7x7 board with some moves that might lead
    # to repeated states if the search is deep enough or branches enough.
    # We'll place a few X's and O's. Corners are forbidden '-' as usual in FIAR.
    ...
    partial_state = game_types.State(
        initial_state_data=[partial_board, "X"]
    )

    # Prepare the agent for FIAR:
    agent.prepare(game_types.FIAR, what_side_to_play="X", opponent_nickname="Z-Tester",
                  expected_time_per_move=1.0, utterances_matter=False, apis_ok=False)

    # Now call make_move on this partial board with alpha-beta and Zobrist turned on,
    # and with a bigger max_ply so there's a decent sized tree:
    response = agent.make_move(
        current_state=partial_state,
        current_remark="Testing Zobrist in FIAR",
        time_limit=999,
        autograding=False,
        use_alpha_beta=True,
        use_zobrist_hashing=True,
        max_ply=4,   # or even 5 if you want deeper 
        special_static_eval_fn=None
    )

    move_and_state, agent_utterance = response
    chosen_move, resulting_state = move_and_state

    print("Zobrist test on FIAR gave this result:")
    print("Chosen move:", chosen_move)
    print("Agent remark:", agent_utterance)
    print("Resulting hits/writes should appear in that remark if you track them.")

    
if __name__ == "__main__":

    agent = agent_module.OurAgent()
    test_zobrist_hits_on_fiar(agent)
